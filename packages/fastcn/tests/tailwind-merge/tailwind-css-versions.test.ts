...
test("supports Tailwind CSS v3.3 features", () => {
  ...
  expect(
    twMerge("start-0 end-0 inset-0 ps-0 pe-0 p-0 ms-0 me-0 m-0 rounded-ss rounded-es rounded-s"),
...
test("supports Tailwind CSS v4.3 container query container features", () => {
  ...
  expect(twMerge("@container-size/sidebar @container")).toBe("@container-size/sidebar @container");
