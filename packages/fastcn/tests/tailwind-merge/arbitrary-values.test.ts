...
test("handles simple conflicts with arbitrary values correctly", () => {
  ...
  expect(twMerge("h-[10px] h-[11cqw] h-[12cqh] h-[13cqi] h-[14cqb] h-[15cqmin] h-[16cqmax]")).toBe(
    "h-[16cqmax]",
  );
...
test("handles ambiguous arbitrary values correctly", () => {
  ...
  expect(
    twMerge("bg-none bg-[url(.)] bg-[image:.] bg-[url:.] bg-[linear-gradient(.)] bg-linear-to-r"),
  ).toBe("bg-linear-to-r");
  expect(
...
test("handles arbitrary custom properties correctly", () => {
  ...
  expect(twMerge("font-(--a) font-(--b)")).toBe("font-(--b)");
  expect(twMerge("font-(weight:--a) font-(--b)")).toBe("font-(--b)");
  expect(twMerge("font-(family-name:--a) font-(--b)")).toBe("font-(family-name:--a) font-(--b)");
});
